...
import br.com.netshoes.databinding.DetailBinding
...
class DetailFragment : HelpFragment(){
    ...
    fun initData() {
        /*dt_cb_favorites.setOnClickListener {
            ...
            if ((it as CheckBox).isChecked) {
                ...
                viewModel.addGists(gistsPO)
            } else {
                Constant.database!!.gistsDao().allGists().forEach { it ->
                    if (gists.id == it.id) {
                        viewModel.deleteGists(it)
                    }
                }
            }
        }*/
